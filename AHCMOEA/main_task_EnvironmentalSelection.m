function [return_pop,return_Fitness] = main_task_EnvironmentalSelection(Population,N,Set_cons,true,VAR)
    %---------------------------------------------------------------------%
    ...
    if isempty(fPopulation)
        ifFitness = CalFitness(ifPopulation.objs,ifPopulation.cons,Set_cons,true);
        Next2 = ifFitness < 1;
        if sum(Next2) <= N
            [~,Rank] = sort(ifFitness);
            Next2(Rank(1:N )) = true;
    ...
    elseif length(fPopulation) <= N
        ...
        cons(cons<0)=0;
        cons = sum(cons,2);
        fFitness = CalFitness(fPopulation.objs,cons,Set_cons,true);
        Next = fFitness < 1;
